...
/* This object mirrors the response shape expected from the future backend. */
const fakeResult = {
  task: 'change_detection',
  answer: 'Built-up area increased primarily in the northern portion of the scene.',
  confidence: 0.87,
  analysisType: 'Change Analysis',
  workflow: 'Bi-temporal Analysis',
  models: ['Change Detection', 'NDVI Analysis', 'Gemini'],
  parameters: {
    ndvi: true,
    threshold: 0.30
  },
  evidence: {
    type: 'change_map',
    caption: 'Highlighted regions indicate detected change clusters returned by the analysis workflow.',
    boxes: [
      { id: 'CHG_01', x: 16, y: 22, width: 25, height: 27 },
      { id: 'CHG_02', x: 57, y: 48, width: 22, height: 25 }
    ]
  },
  timeline: [
    'Input validated',
    'Task identified',
    'Change Detection selected',
    'NDVI analysis executed',
...
function renderEvidence(result) {
  ...
  const backendEvidenceImage = evidence.imageUrl;
  if (backendEvidenceImage || (file && isDecodable(file))) {
    ...
    const boxes = Array.isArray(evidence.boxes) ? evidence.boxes : [];
    boxes.forEach((box) => {
...
function renderAnalysisResult(result, query) {
  ...
  document.getElementById('executionParameters').textContent =
    `NDVI: ${result.parameters.ndvi ? 'Enabled' : 'Disabled'} · Threshold: ${result.parameters.threshold.toFixed(2)}`;
...
function analysisTypeLabel(task) {
  const labels = {
    feature_mapping: 'Feature Mapping',
    change_detection: 'Change Analysis',
    single_image: 'Visual Question Answering',
    ndvi_analysis: 'Vegetation / NDVI Analysis'
  };
  return labels[task] || task || 'Analysis';
}

function normalizeBackendResult(data) {
  const evidence = data.visual_evidence || {};
  const audit = data.audit_trail || {};
  const models = Array.isArray(audit.models_used) && audit.models_used.length
    ? audit.models_used
    : [evidence.type === 'feature_mask' ? 'Feature Mapper' : 'SatQuery Backend'];
  const parameters = audit.parameters || {};
  const timeline = [];

  if (audit.routing_decision) timeline.push(`Routing: ${audit.routing_decision}`);
  if (Array.isArray(audit.preprocessing_steps)) timeline.push(...audit.preprocessing_steps);
  timeline.push(`${analysisTypeLabel(data.task)} completed`);

  const imageUrl = evidence.url ? `${API_BASE}${evidence.url}` : null;
  const featureBits = [];
  if (typeof evidence.count === 'number') featureBits.push(`${evidence.count} detected region${evidence.count === 1 ? '' : 's'}`);
  if (typeof evidence.coverage_pct === 'number') featureBits.push(`${evidence.coverage_pct}% coverage`);

  return {
    task: data.task,
    answer: data.answer,
    confidence: typeof data.confidence === 'number' ? data.confidence : 0.7,
    analysisType: analysisTypeLabel(data.task),
    workflow: modeLabel(),
    models,
    parameters: {
      ndvi: Boolean(parameters.ndvi),
      threshold: typeof parameters.threshold === 'number' ? parameters.threshold : 0
    },
    evidence: {
      type: evidence.type,
      imageUrl,
      maskUrl: imageUrl,
      state: evidence.type === 'feature_mask' ? 'Feature mask returned' : 'Visual evidence returned',
      caption: featureBits.length
        ? featureBits.join(' · ')
        : 'Visual evidence from analyzed imagery.'
    },
    timeline
  };
}

async function runBackendAnalysis(query) {
  const formData = new FormData();
  selectedFiles.forEach((file) => formData.append('files', file, file.name));
  const modalityHints = selectedFiles.map((file) => {
    const role = fileRoles.get(file);
    if (role === 'sar') return 'sar';
    return 'optical';
  });
  formData.append('modality_hints', JSON.stringify(modalityHints));
  ...
  return normalizeBackendResult(await queryResponse.json());
}
...
function simulateQuery() {
  ...
  const stageTimes = [350, 450, 1600, 700, 500];
  const total = stageTimes.reduce((a, b) => a + b, 0);

  let acc = 0;
  stageTimes.forEach((dur, i) => {
    setTimeout(() => {
      renderStages(i + 1);
      if (i === stageTimes.length - 1) {
        setTimeout(() => {
          try {
            renderAnalysisResult(fakeResult, query);
          } catch (error) {
            hideLoading();
           ...
           queryCount += 1;
           const latency = ((Math.random() * 800 + 150) + total / 1000).toFixed(0);
...
runBtn.addEventListener('click', simulateQuery);
